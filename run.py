...
# $Id: run.py 20368 2026-02-20 08:01:32Z ottevan $ 
# 
#===================================================================
# Import modules
#===================================================================
import sys,os,glob,time,adaptsrc
...
def getmodelrefplane(Trim2DepExe,workdir,RunIDs,fromfil,fromidx,tofil):
...
def updaterefplane(workdir,RunIDs,modelrefplane,refplane):
    print('Updating reference plane')
    worksep = workdir + os.sep
    for runid in RunIDs:
      print('Reading grid file for dimensions')
      gridfile = worksep+runid+'.grd'
      print(gridfile)
      modelgrid = Grid.fromfile(gridfile)
      sys.stdout.flush()
      #print runid, modelgrid.shape #debugging
      if os.path.exists(worksep+refplane+runid+'.dep'):
        print('Renaming '+worksep+refplane+runid+'.dep'+ ' to '+worksep+refplane+runid+'.old')
        sys.stdout.flush()
        move(worksep+refplane+runid+'.dep',worksep+refplane+runid+'.old')
      elif os.path.exists(worksep+modelrefplane+runid+'.dep'):
        print('Reference plane does not exist') 
        print('Using model reference plane as reference plane') 
        print('Renaming '+worksep+modelrefplane+runid+'.dep'+ ' to '+worksep+refplane+runid+'.dep')
        print('Renaming '+worksep+modelrefplane+runid+'.dep'+ ' to '+worksep+refplane+runid+'.dep')
        sys.stdout.flush()
      if os.path.exists(worksep+modelrefplane+runid+'.old'):
        print('Model reference plane from previous OLA discharge exists')
        # read
        depMRO  = Dep.read(worksep+modelrefplane+runid+'.old',modelgrid.shape)
        depMR   = Dep.read(worksep+modelrefplane+runid+'.dep',modelgrid.shape)
        depOLRO = Dep.read(worksep+refplane+runid+'.old',modelgrid.shape)
        depOLR  = depOLRO
        depOLR.val = depOLRO.val + depMR.val - depMRO.val
        Dep.write(depOLR,worksep+refplane+runid+'.dep')
      else:
        print('Model reference plane from previous OLA discharge does not exist')
        print('Reference plane is not updated')
        sys.stdout.flush()
        move(worksep+refplane+runid+'.old',worksep+refplane+runid+'.dep')
...
def run(RunIDs,TrisimParameters,TStart,TStop,Disch,Src,OLA):
  ...
  if arch == 'lnx64':
    platform_extension = '.sh'
  else:
    platform_extension = '.bat'
  
  FlowExe = execpath+'run_dflow2d3d'+platform_extension
  QdbaseExe = execpath+'run_qdb'+platform_extension
  Trim2DepExe = execpath+'run_trim2dep'+platform_extension

  
  #-----------------------------------------------------------------
  # Verify existence of executables
  #-----------------------------------------------------------------
  Executables = [QdbaseExe,Trim2DepExe,FlowExe]
  ...
  if os.path.exists(ToBackUp):
    ...
    if InitialRun:
      ...
      #except: 
      print('Warning: no sediment found, restarting hydrodynamics only ...')
      qdbcopy(QdbaseExe,workdir,RunIDs,
        ['map-info-series','map-sed-series','map-infsed-serie'],
        up+FromBackUp+os.sep+'trim-',-1,restartfile,1)
  ...
  #-----------------------------------------------------------------
  # Copy model reference plane
  #-----------------------------------------------------------------
  if not InitialRun:
    worksep = workdir + os.sep
    for runid in RunIDs:
      refplanefiles = glob.glob(FromBackUp+os.sep+'modelrefplane'+runid+'.dep')
      for file in refplanefiles:
        if os.path.exists(worksep+'modelrefplane'+runid+'.dep'):
          move(worksep+'modelrefplane'+runid+'.dep',worksep+'modelrefplane'+runid+'.old')
        print('Copying Model Reference Plane for domain "'+runid+'" to work directory.')
        sys.stdout.flush()
        copy(file,workdir)
  ...
  #-----------------------------------------------------------------
  if OLA == 'Y': 
    getmodelrefplane(Trim2DepExe,workdir,RunIDs,'trim-','-1','modelrefplane')
    updaterefplane(workdir,RunIDs,'modelrefplane','refplane')
  ...
  #-----------------------------------------------------------------
  if not InitialRun:
    if TraceBackInfo == 0 or TraceBackInfo == 1:
      filetype = 'all files'
